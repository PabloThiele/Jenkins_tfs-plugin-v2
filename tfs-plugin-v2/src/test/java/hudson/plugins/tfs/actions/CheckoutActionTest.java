...
public class CheckoutActionTest {
    ...
    public void assertFirstCheckoutBySingleVersionSpecNotUsingUpdate() throws Exception {
    	...
    	when(workspaces.exists("workspace", true, true)).thenReturn(true).thenReturn(false);
    	when(workspaces.newWorkspace(eq("workspace"), eq("project"), isA(String.class))).thenReturn(workspace);
    	when(workspaces.getWorkspace("workspace", true, true)).thenReturn(workspace);
    	
    	new CheckoutAction("workspace", "project", ".", false, true, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertFirstCheckoutNotUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", ".", false, true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertFirstCheckoutBySingleVersionSpecUsingUpdate() throws Exception {
        ...
        when(server.getProject("project")).thenReturn(project);
        when(project.getProjectPath()).thenReturn("project");
        when(workspaces.exists(new Workspace("workspace"), true, true)).thenReturn(false);
        when(workspaces.newWorkspace(eq("workspace"), eq("project"), isA(String.class))).thenReturn(workspace);
        
        new CheckoutAction("workspace", "project", ".", true, true, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertFirstCheckoutUsingUpdate() throws Exception {
    	...
    	new CheckoutAction("workspace", "project", ".", true, true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertSecondCheckoutBySingleVersionSpecUsingUpdate() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(server.getProject("project")).thenReturn(project);
        when(workspaces.exists("workspace", true, true)).thenReturn(true);
        when(workspaces.getWorkspace("workspace", true, true)).thenReturn(workspace);
    ...
    public void assertDetailedHistoryIsRetrievedInSecondBuildCheckingOutByLabel() throws Exception {
        ...
        CheckoutAction action = new CheckoutAction("workspace", "project", ".", true, true, true);
    ...
    public void assertWorkFolderIsCleanedIfNotUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", "tfs-ws", false, true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertWorkFolderIsCleanedIfNotUsingUpdateCheckingOutByLabel() throws Exception {
        ...
        new CheckoutAction("workspace", "project", "tfs-ws", false, true, true).checkoutBySingleVersionSpec(server, hudsonWs, MY_LABEL);
    ...
    public void assertWorkspaceIsNotCleanedIfUsingUpdate() throws Exception {
        ...
        new CheckoutAction("workspace", "project", "tfs-ws", true, true, true).checkout(server, hudsonWs, null, Util.getCalendar(2009, 9, 24));
    ...
    public void assertCheckoutBySingleVersionSpecDeletesWorkspaceAtStartIfNotUsingUpdate() throws Exception {
        ...
        verify(workspaces, times(2)).exists("workspace", true, true);
        verify(workspaces).getWorkspace("workspace", true, true);
        verify(workspaces).deleteWorkspace(workspace);
        verify(workspaces).newWorkspace(eq("workspace"), eq("project"), isA(String.class));
    ...
    public void assertCheckoutDoesNotDeleteWorkspaceAtStartIfUsingUpdate() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(workspaces.exists("workspace", true, true)).thenReturn(true).thenReturn(true);
        when(workspaces.getWorkspace("workspace", true, true)).thenReturn(workspace);
        when(server.getProject("project")).thenReturn(project);
    ...
    public void assertCheckoutDoesNotDeleteWorkspaceIfNotUsingUpdateAndThereIsNoWorkspace() throws Exception {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(workspaces.exists("workspace", true, true)).thenReturn(false).thenReturn(false);
        when(workspaces.newWorkspace(eq("workspace"), eq("project"), isA(String.class))).thenReturn(workspace);
        when(server.getProject("project")).thenReturn(project);
        when(project.getProjectPath()).thenReturn("project");
