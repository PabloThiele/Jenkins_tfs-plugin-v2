...
public class RemoveWorkspaceActionTest {
    ...
    public void assertNoSuchWorkspaceNameDoesNothing() throws Exception {
        ...
        when(workspaces.exists(anyString(), anyBoolean(), anyBoolean())).thenReturn(false);
        
        RemoveWorkspaceAction action = new RemoveWorkspaceAction("workspace");
        assertThat(action.remove(server, true, true), is(false));

        verify(server).getWorkspaces();
        verify(workspaces).exists("workspace", true, true);
    ...
    public void assertWorkspaceIsDeleted() throws Exception  {
        when(server.getWorkspaces()).thenReturn(workspaces);
        when(workspaces.exists(anyString(), anyBoolean(), anyBoolean())).thenReturn(true);
        when(workspaces.getWorkspace(anyString(), anyBoolean(), anyBoolean())).thenReturn(workspace);
        
        RemoveWorkspaceAction action = new RemoveWorkspaceAction("workspace");
        assertThat(action.remove(server, true, true), is(true));
