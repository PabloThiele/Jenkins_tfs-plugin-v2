...
public class ListWorkspacesCommand extends AbstractCallableCommand implements Callable<List<Workspace>, Exception> {
    ...
    public ListWorkspacesCommand(final ServerConfigurationProvider server, final boolean showWorkspaces, final boolean useTFSUserNameOnSearch) {
        this(server, null, showWorkspaces, useTFSUserNameOnSearch);
    }

    public ListWorkspacesCommand(final ServerConfigurationProvider server, final String computer, final boolean showWorkspaces, final boolean useTFSUserNameOnSearch) {
        super(server);
    ...
    public List<Workspace> call() throws Exception {
        ...
        String tfsUser = null;
        if(useTFSUserNameOnSearch){
            tfsUser = System.getProperty("TFS_USERNAME");
        }
