...
public class Workspaces implements ListWorkspacesCommand.WorkspaceFactory {
     ...
     */
    private List<Workspace> getListFromServer(boolean showWorkspaces, boolean useTFSUserNameOnSearch) {
        ListWorkspacesCommand command = new ListWorkspacesCommand(server, showWorkspaces, useTFSUserNameOnSearch);
     ...
     */
    private void populateMapFromServer(boolean showWorkspaces, boolean useTFSUserNameOnSearch) {
        if (!mapIsPopulatedFromServer) {
            for (Workspace workspace : getListFromServer(showWorkspaces, useTFSUserNameOnSearch)) {
     ...
     */
    public Workspace getWorkspace(String workspaceName, boolean showWorkspaces, boolean useTFSUserNameOnSearch) {
        if (!workspaces.containsKey(workspaceName)) {
            populateMapFromServer(showWorkspaces, useTFSUserNameOnSearch);
        }
     ...
     */
    public boolean exists(String workspaceName, boolean showWorkspaces, boolean useTFSUserNameOnSearch) {
        if (!workspaces.containsKey(workspaceName)) {
            populateMapFromServer(showWorkspaces, useTFSUserNameOnSearch);
        }
     ...
     * @return true if the workspace exists on server; false otherwise
     */
    public boolean exists(Workspace workspace, boolean showWorkspaces, boolean useTFSUserNameOnSearch) {
        return exists(workspace.getName(), showWorkspaces, useTFSUserNameOnSearch);
    }
